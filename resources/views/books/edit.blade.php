<x-layouts.app>
    ...
    <form action="{{ route('books.update', $book->id) }}" method="POST" class="mt-4 space-y-4">
        @csrf
        @method('PUT')
        <div class="space-y-1">
        ...
        <div class="space-y-1">
            ...
            <input type="text" name="author" id="author" class="rounded block w-full" value="{{ old('title') ?: $book->author }}">
            @error('author')
            <div class="text-red-500 text-sm">
                {{ $message }}
            </div>
